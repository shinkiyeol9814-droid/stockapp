...
- `get_corp_map()`의 corpCode.xml(3.6MB) 다운로드는 종목별 시간 예산이 시작되기 **전**에
  실행돼 그 자체엔 상한이 없었다 — requests read timeout은 바이트 간격만 재서 느리게
  트리클되면 안 걸리고, 앱 요청 1건(원래 수십 초)이 이 다운로드 하나 때문에 4분 넘게
  걸린 적이 있다. 벽시계 예산(30초, `_CORPMAP_BUDGET`)으로 감쌌고, `corp_map.json`은
  `.gitignore`돼 있어 실행마다 새로 받으므로 `dart_batch.yml`이 주 단위 키로 GitHub Actions
  캐시에 담아 대부분의 실행이 다운로드 자체를 건너뛰게 한다.
...
절전 임계값은 **12시간**이다(Streamlit 측 포럼 답변 기준. 7일 → 2~3일 → 12시간으로 짧아졌고
공식 문서에는 수치가 없다). 하루 1회 핑으로는 실행 사이에 반드시 잠든다.

`keep_alive.yml`이 3시간마다 헤드리스 Chromium으로 실제 접속해서, 잠들어 있으면
"get this app back up" 버튼을 누르고 본문이 렌더링되는지까지 확인한다.
확인 실패 시 실행을 실패로 표시하고 스크린샷을 아티팩트로 남긴다.

### 절전과 리디렉션 루프는 다른 문제다
`ERR_TOO_MANY_REDIRECTS`(share.streamlit.io)는 절전이 아니다. Streamlit Cloud는
앱 도메인 → `share.streamlit.io/-/auth/app` → 앱 도메인 순으로 인증 쿠키를 심는데,
**브라우저가 그 쿠키를 거부하면 무한 반복**된다(크롬 모바일의 서드파티 쿠키 차단이 흔한 원인).
쿠키를 막고 curl로 받으면 그대로 재현되고(303이 10회 반복), 쿠키를 켜면 3회 만에 200으로 끝난다.

사용자 쪽 해결은 해당 도메인 쿠키 삭제 / 시크릿 탭 / `[*.]streamlit.app` 서드파티 쿠키 허용,
그래도 안 되면 앱 재부팅이다. keep-alive는 이와 무관하지만, 인증 흐름 자체가 깨져 **모든**
사용자가 못 들어가는 경우를 잡으려고 쿠키를 켠 상태의 리디렉션 사슬(정상: 3회 → 200)을
같이 점검한다.
