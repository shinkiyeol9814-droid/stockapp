...
# ==========================================
# 💡 좌측 사이드바: 네비게이션 메뉴 구성 (복구 완료!)
# ==========================================
st.sidebar.title("🧭 네비게이션 메뉴")
menu = st.sidebar.radio(
    "메뉴 선택",
    ["📈 밸류에이션 (PER/POR밴드)", "🛠️ 버전 업데이트 이력"],
        ...
        with st.spinner("데이터 분석 중..."):
            ...
            if ticker_row.empty:
                ...
                if not df_price.empty:
                    ...
                    if pd.notna(today_mult):
                        fig.add_annotation(x=df_price.index[-1], y=curr_p, text=f"현재: {today_mult:.1f}x", showarrow=True, arrowhead=2, ax=-40, ay=-30, font=dict(size=12, color="red", weight="bold"), bgcolor="rgba(255,255,255,0.8)", bordercolor="red", borderwidth=1, row=1, col=1)
                    if tp1 > 0:
                        fig.add_annotation(x=fin_df[fin_df['Year'] == year_1]['Plot_Date'].iloc[0], y=tp1, text=f"목표: {target_mult:.1f}x", showarrow=True, arrowhead=2, ax=-40, ay=-30, font=dict(size=12, color="blue", weight="bold"), bgcolor="rgba(255,255,255,0.8)", bordercolor="blue", borderwidth=1, row=1, col=1)
                    ...
                    fig.update_layout(
                        ...
                        margin=dict(l=0, r=0, t=30, b=0) # 💡 좌측 마진 0 강제 적용
                    )

                    # 💡 모바일 스크롤 방해 금지 (Static)
                    st.plotly_chart(fig, use_container_width=True, config={'staticPlot': True, 'displayModeBar': False})
                    st.caption("🔍 **Tip:** 우측 상단의 **[⛶ 전체화면]** 아이콘을 눌러 그래프를 크게 보세요.")

    else:
        st.info("👆 상단 검색창에 종목명을 입력해주세요! (예: 삼성전자)")
...
# ==========================================
    ...
    df_history = pd.DataFrame({
        "버전": ["V1.1.3 (메뉴 픽스)", "V1.1.2 (가시성 극대화)", "V1.1.0", "V1.0.4", "V1.0.0"],
        "업데이트 내용": [
            "사이드바 메뉴 누락 버그 픽스",
