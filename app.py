...
def parse_and_filter_html(html):
    ...
    for idx in filtered_df.index:
        if re.search(r'^(매출액|영업수익|순영업수익)', idx):
            rename_dict[idx] = '매출액'
        elif re.search(r'^영업이익$', idx):
            rename_dict[idx] = '영업이익'
        elif re.search(r'^영업이익\(발표기준\)', idx):
            rename_dict[idx] = '영업이익(발표기준)'
        elif re.search(r'^당기순이익', idx):
            rename_dict[idx] = '당기순이익'
        elif re.search(r'^PER', idx):
            rename_dict[idx] = 'PER(배)'
        elif re.search(r'^PBR', idx):
            rename_dict[idx] = 'PBR(배)'
        ...
        if fin is not None and not fin.empty:
            ...
            for d, row in fin.iterrows():
                ...
                if y in target_years:
                    rev, op, ni = row.get('Total Revenue', np.nan), row.get('Operating Income', np.nan), row.get(
                        'Net Income', np.nan)
                    ...
                    if y in target_years:
                        def get_val(item_name):
                            if item_name in df_annual.index:
                                ...
                                try:
                                    return float(re.sub(r'[^\d\.-]', '', str(val))) if pd.notna(val) and str(
                                        val).strip() not in ['', '-', 'N/A'] else np.nan
                                except:
                                    return np.nan
                            return np.nan
        ...
        with st.spinner(f"'{corp_name}' 데이터 수집 중 (가상 브라우저 작동, 약 5~8초 소요)... 🏃‍♂️"):
            ...
            if ticker_row.empty:
                ...
                if df_price.empty or fin_df_cache is None or fin_df_cache.empty:
                    ...
                    fin_df['Plot_Metric'] = fin_df['Metric_Per_Share'].apply(
                        lambda x: 0.001 if pd.isna(x) or x <= 0 else x)

                    fin_df_hist = fin_df[fin_df['Year'] <= 2024].copy()
                    historical_metric_dict = {row['Plot_Date'].year: row['Metric_Per_Share'] for idx, row in
                                              fin_df_hist.iterrows()}
                    ...
                    daily_metrics_interp = np.interp(df_price.index.map(datetime.timestamp).values, band_dates_ts,
                                                     band_metrics)

                    df_price['Est_Metric'] = daily_metrics_interp
                    df_price['Current_Valuation'] = np.where(df_price['Est_Metric'] <= 0.002, np.nan,
                                                             df_price['Close'] / df_price['Est_Metric'])

                    today_est_metric = df_price['Est_Metric'].iloc[-1]
                    today_mult = current_price / today_est_metric if pd.notna(
                        today_est_metric) and today_est_metric > 0.002 else np.nan
                    ...
                    with col_m1:
                        st.metric(label=f"현재 주가 ({df_price.index[-1].strftime('%Y-%m-%d')})",
                                  value=f"{current_price:,.0f} 원",
                                  delta=f"{current_price - prev_close:,.0f} 원 ({(current_price / prev_close - 1) * 100:.2f}%)")
                        if marcap > 0:
                            st.markdown(
                                f"<span style='color:gray; font-size:16px;'>**시가총액:** {marcap / 1_000_000_000_000:.2f}조 원</span>",
                    ...
                    fig.add_trace(go.Scatter(x=df_price.index, y=df_price['Close'], mode='lines', name='현재 주가',
                                             line=dict(color='black', width=2)), row=1, col=1)

                    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#9467bd']
                    if bands:
                        for i, b in enumerate(bands):
                            fig.add_trace(go.Scatter(x=extended_dates, y=extended_metrics_interp * b, mode='lines',
                                                     name=f'{b:.1f}x',
                                                     line=dict(color=colors[i % len(colors)], width=1, dash='dot')),
                                          row=1, col=1)

                    if pd.notna(today_mult):
                        fig.add_trace(go.Scatter(x=extended_dates, y=extended_metrics_interp * today_mult, mode='lines',
                                                 name=f'실시간 가치({today_mult:.1f}x)', line=dict(color='red', width=2.5)),
                                      row=1, col=1)

                    fig.add_trace(go.Scatter(x=extended_dates, y=extended_metrics_interp * target_mult, mode='lines',
                                             name=f'목표 가치({target_mult}x)', line=dict(color='blue', width=2.5)), row=1,
                                  col=1)
                    ...
                    fig.add_trace(go.Scatter(x=valid_price.index, y=valid_price['Current_Valuation'], mode='lines',
                                             name=f'당일 {band_name}', line=dict(color='purple', width=1.5)), row=2,
                                  col=1)
                    ...
                    fig.update_yaxes(range=[0, max(max_mult * 1.1 if max_mult > 0 else 30, target_mult * 1.2)],
                                     title_text=f"{band_name} (배)", row=2, col=1)
...
# ==========================================
    ...
    history_data = {
        "버전": ["V39.1", "V39.0", "V38.0", "V37.0", "V36.0", "V35.0", "V34.0", "V30.2"],
        "업데이트 내용": [
